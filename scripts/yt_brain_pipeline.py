...
def fetch_youtube_metadata(url):
    ...
    try:
        ...
        print(f"Warning: yt-dlp execution error: {e}")
        # Fallback dummy video for testing
        return [{
...
def score_and_summarize_with_gemini(api_key, video):
    ...
    try:
        ...
        return {
            ...
            "channel": video["channel"],
            "url": video["url"],
            "summary": video["description"][:200] if video["description"] else "Praktické video zaměřené na AI automatizace.",
            "action": "Prohlédnout si video",
            "tags": "#AI #Automation",
            "score": 8,
            "category": "AI / WORK",
            "key_points": ["Užitečné informace pro vývoj a automatizaci"]
        }
...
def main():
    ...
    target_url = args.playlist or args.video or "https://www.youtube.com/watch?v=sBF3UumkL4Y"
    api_key = get_env_gemini_key()
    if not api_key:
        print("Error: GEMINI_API_KEY or IVCA_GEMINI_API is not set!")
        return
    ...
    processed_items = []
    for v in videos[:10]: # limit to top 10 items
    ...
    brief_data = {
        "date": today_str,
        "text": f"Dobré ráno Ivo! V dnešním AIVOS Brain Briefu máme {len(processed_items)} zpracovaných videí a návodů z YouTube.",
        "stats": {
            "high": len(high_items),
    ...
    print(f"[3/3] Successfully generated daily brief for {today_str} in public/briefs/!")
